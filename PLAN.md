...
**黑板本质**：项目组内部拉平需求和进度、头脑风暴、工程方向提示的工具。就像侦探办案——不同侦探在黑板上写下自己调查到的线索，最后汇总成项目的实现路径。
...
2. **项目匹配**：CEO无需提及具体项目名或UUID → LLM 根据描述匹配项目
   - 遍历数据库中已完成和未完成的项目
   - 每个项目的第一条黑板记录包含：① 项目需求的原始输入（来自CEO）② 该项目的需求梳理
   - LLM 比对CEO描述与黑板第一条记录 → 返回最匹配的项目 UUID
...
│  │  ┌──────────────────────────────────────────────┐   │  │
│  │  │ ChromaDB (嵌入向量索引)                       │   │  │
│  │  │ - blackboard_entries.embedding → 语义检索     │   │  │
│  │  │ - 写入时自动生成 embedding                    │   │  │
│  │  │ - 读取时支持相似度搜索                        │   │  │
│  │  └──────────────────────────────────────────────┘   │  │
│  └────────────────────────────────────────────────────┘  │
...
> 💡 以下是上下文注入的**逻辑分组**（5层），实际 System Prompt 拼装见 §4.2 的**8层完整结构**。两者是同一机制的不同粒度描述：5层是逻辑归类，8层是运行时拼装顺序。
...
*最后更新: 2026-05-02 (v7: PM验收+并行需求变更+AgentServer意图识别+项目UUID+LLM配置+安全边界+错误恢复+Agent ID命名+性能基线)*
