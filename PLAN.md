...
|------|------------|-----------------|
| 智慧与战略 | 统筹全局，制定策略 | AgentServer (投资人) 统筹公司方向 |
...
**Athena 是一个像 IT 公司一样运作的 AI Agent 编排系统**——用户开箱即用，通过 Web 界面下达项目需求，系统自动招聘专业 Agent、分配任务、协调推进，每个 Agent 上下文隔离、专人专用，通过黑板模式共享项目知识。
...
| 组件 | 职责 |
|------|------|
| **AgentServer (投资人)** | 与用户对话、获取需求、关键抉择 |
| **HR Agent** | 感知能力缺口、按模板创建新 Agent、模板不足时组织小组编写新模板 |
| **项目经理 Agent** | 拆解和细化项目需求（参考 Hermes、腾讯 WorkBuddy 等 Agent 的任务拆解方式） |
...
### 6. Agent 组会与自动沟通
当项目遇到问题或不确定节点时：
1. 发现问题的 Agent 联系相关人员（联系方法写在 soul 文件里）
2. 先确认信息来源（员工/互联网/其他）
3. 结合其他信息判断（如"这个信息是其他员工在会议上告诉他的，则进一步和该员工沟通"）
4. 如果当事人无法沟通出结果（如原始需求确实模糊）→ 上报项目经理
5. 项目经理决定：
   - 自动调研并采取最主流做法，或
   - 给出合适的选项询问 AgentServer → 展示给用户选择
6. 用户未选择时，相关 Agent 先去完成公司中其他任务

### 7. Agent 相互沟通机制
Agent 要勤于与其他 Agent 沟通项目，多讨论：
- 对方知道相关问题的答案 → 告知答案、搜寻方法、思考模式
- 接收方觉得有价值 → 写入自己的 memory.md
...
- 商讨仍无法消除不确定 → 汇报投资人 Agent (AgentServer)
- 投资人 Agent 向用户发起会话，用户作为公司管理人员仅进行关键抉择

### 9. 开箱即用
用户安装后即可通过 Web 界面使用，无需复杂配置

---

## ⚠️ 设计矛盾与待确认项

> 以下为当前设计中存在的矛盾或待确认点，需要审查后决定。

### 矛盾 1: "Agent 不直接通信" vs "Agent 勤于沟通开会"

| 条目 | 原设计 | 新需求 |
|------|--------|--------|
| 旧规则 | "Agent 之间不直接通信，所有知识交换通过黑板" | 新增: Agent 勤于沟通、多对多讨论、会议决议分发 |

**建议解决方案**: 区分"知识持久化"和"实时沟通"两个通道——
- **知识持久化** → 仍然通过黑板（事实、决议、进展等写入黑板）
- **实时沟通** → 通过会议系统（临时数据库 + 决议生成），这是结构化的沟通而非自由对话
- 两个通道都不允许 Agent 直接读取对方的个人工作记忆

### 矛盾 2: 会议数据归属

会议临时数据库存哪里？
- 选项 A: 独立的 SQLite 数据库 (meetings.db)
- 选项 B: 黑板中的一个层级
- **建议**: 选项 A，会议数据生命周期短（开完会就删），不适合放黑板

### 矛盾 3: 沟通频率 vs 上下文隔离

"勤于沟通"和"上下文隔离"的边界？
- **建议**: 沟通产出（决议、答案）写入黑板或 memory.md，但沟通过程（思考、讨论）不进入对方上下文

### 待确认 4: 项目经理 Agent 角色

一句话定义中新增了"项目经理 Agent"，但角色表和黑板权限矩阵中缺少对应行。
- **建议**: 添加项目经理 Agent 到角色体系，职责为"拆解需求、分配任务、处理不确定性上报"

### 待确认 5: 投资人 Agent = AgentServer?

第8条提到"汇报投资人agent"，系统架构中 AgentServer 是 CEO/投资人。
- **建议**: 明确 AgentServer = 投资人 Agent，统一称呼
...
| **会议系统** | 独立 meetings.db (SQLite) | 会议数据生命周期短，独立于黑板 |
...
#### 10. meetings — 会议表 (独立 meetings.db)
...
#### 11. meeting_participants — 会议参与者表 (meetings.db)
...
#### 12. meeting_messages — 会议发言表 (meetings.db)
...
| 角色 | 职责 | 专属工具集 | 上下文内容 |
|------|------|-----------|-----------|
| **AgentServer (投资人)** | 与用户对话、获取需求、关键抉择 | 项目管理工具、HR 调度工具、用户交互 | 全局视角 |
...
```
1. AgentServer/项目经理 评估需求 → 发现缺少某种能力的 Agent
2. 向 HR Agent 发出招聘请求:
   {
     "role": "developer",
     "project_id": "proj-xxx",
...
```
1. 发起 → Agent 发现需要讨论的问题
2. 创建会议 → 写入 meetings.db, need_resolution=1
...
规则 4: 每个Agent只能看到黑板中自己有权限的部分
规则 5: 任务分配通过项目经理 Agent
规则 6: 不确定问题处理链路: Agent → 联系相关人 → 项目经理 → 投资人 → 用户
规则 7: 会议讨论过程不对非参与者可见，只有决议写入黑板后所有相关人可见
```
...
// 用户抉择
GET  /api/projects/{id}/decisions     # 获取待用户抉择的选项
POST /api/projects/{id}/decisions     # 用户做出抉择
...
│   │
│   ├── core/                     # 核心模块
│   │   ├── agent_server.go       # AgentServer (投资人/CEO)
