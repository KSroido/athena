...
**Athena 是一个像 IT 公司一样运作的 AI Agent 编排系统**——用户开箱即用，通过 Web 界面下达项目需求，系统自动招聘专业 Agent、分配任务、协调推进，每个 Agent 上下文隔离、专人专用，遇到问题立刻开会沟通对齐，确保软件工程每个环节都有专人负责，交付稳定产品。
...
| 组件 | 职责 |
|------|------|
| **AgentServer (投资人)** | 与用户直接对话、获取项目、关键抉择。项目交给项目经理 Agent 拆解 |
...
1. 用户通过 Web 界面下达需求 → AgentServer 接收
...
3. 多人无法达成一致 → 上报项目经理
4. 项目经理无法决策 → 上报 AgentServer (投资人) → 展示给用户选择
5. 用户未选择时，相关 Agent 先去完成公司中其他任务
...
- 商讨仍无法消除不确定 → 汇报 AgentServer (投资人)
- AgentServer 向用户发起会话，用户作为公司管理人员仅进行关键抉择

### 10. 公司规模上限
用户可定义公司规模上限，HR 招聘前必须检查：
- **人数上限**：如 "最多 100 个 Agent"
- **资源上限**：如 "总内存不超过 16GB"
- 上限配置由 AgentServer 管理，用户可随时调整
- 达到上限后，HR 暂停招聘，需要扩容时由 AgentServer 向用户确认
...
**用户决定**: 只有遇到问题才沟通，平常上下文完全独立。遇到别人领域的问题立刻开会，绝不自己琢磨。沟通产出（决议）写入黑板，沟通过程（讨论）不进入对方上下文。
...
**用户决定**: 每个会议独立 SQLite 文件 (`meeting_{id}.sqlite`)，与黑板完全分离。会议结束后可归档或删除。

### ✅ 矛盾 3: 沟通频率 vs 上下文隔离 — 已解决

**用户决定**: 只有遇到问题才沟通。平时各 Agent 上下文完全独立，一旦遇到别人领域的问题立刻开会沟通对齐。沟通产出写入黑板，讨论过程不进入对方上下文。
...
### ✅ 待确认 5: AgentServer = 投资人 — 已确认

AgentServer 就是投资人，与用户直接对话，拿到项目后交给项目经理拆解。全文已统一称呼为 "AgentServer (投资人)"。
...
| 角色 | 职责 | 专属工具集 | 上下文内容 |
|------|------|-----------|-----------|
| **AgentServer (投资人)** | 与用户直接对话、获取项目、关键抉择、项目交给项目经理 | 项目管理工具、HR 调度工具、用户交互 | 全局视角 |
...
HR 招聘前必须检查公司规模上限（由用户通过 AgentServer 设定）：
- 当前 Agent 数量 < 人数上限
- 预计新增内存 < 资源上限
- 达到上限 → HR 暂停招聘，通知 AgentServer 向用户申请扩容
...
4. HR 检查公司规模上限 → 超限则上报 AgentServer
5. HR Agent 执行招聘:
...
|------|-------|-------|-------|-------|-------|-------|
| AgentServer (投资人) | RW | RW | RW | R | R | R |
| HR | R | R | R | R | R | R |
...
规则 8: 不确定问题处理链路: Agent → 联系相关人 → 项目经理 → AgentServer (投资人) → 用户
...
│   │
│   ├── core/                     # 核心模块
│   │   ├── agent_server.go       # AgentServer (投资人)
...
│   │
│   ├── templates/                # Agent 模板
│   │   ├── investor.yaml         # 投资人模板 (AgentServer)
│   │   ├── hr.yaml               # HR 模板
│   │   ├── pm.yaml               # 项目经理模板
│   │   ├── developer.yaml
│   │   ├── tester.yaml
...
- [ ] 不确定问题上报链路 (Agent → 相关人 → PM → AgentServer → 用户)
- [ ] 用户抉择 API 与界面
...
3. HR 检查公司规模上限（人数/资源），超限上报 AgentServer
...
*最后更新: 2026-05-02 (v2: 沟通规则重构 + 招聘上限 + 会议独立SQLite + AgentServer=投资人 + PM工作流)*
