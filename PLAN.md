...
| **项目经理 Agent** | 从 AgentServer 接收项目、拆解需求、分配任务、**验收交付**。需要招人时立刻和 HR 沟通（详细说明招什么样的人）。**需求可随时更新（如 Hermes steer）** |
...
7. 项目经理验收交付：项目必须高性能运行，所有测试场景无明显 bug → 验收不通过则要求整改
...
### 12. 完全放权，本地优先
Agent 完全运行在本地，无沙箱限制。文件系统、sudo 等权限不设任何限制，给予 Agent 最大自由度。后续可打包 Docker 镜像，但初期以本地裸跑为主。
...
- **Token 限额**：不主动设置 token 限额，除非 LLM 返回 429（Rate Limit）错误
  - 429 时：提示用户"API 限额了"，暂停整个公司的所有 Agent，等 1 小时后自动重试
  - 重试仍 429 → 继续等待，同时通知CEO
...
3. **降级执行** → 如果 LLM 工具调用失败，降级为命令行 + Python 形式执行
   - 例：文件编辑工具失败 → 降级为 `bash: sed` 或 `python: open().write()`
4. **LLM 调用超时** → 重试 1 次 → 仍超时则暂停该 Agent，通知 PM
5. **Agent goroutine 崩溃** → PM 收到通知，决定是否重启或重新分配任务
...
- **无沙箱**：Agent 直接本地运行，完全放权
- **文件系统**：无访问限制，Agent 可读写任意路径
- **sudo/系统命令**：无限制
- **Docker 打包**：后续考虑，初期不管（写入 TODO）

### 黑板操作鉴权

黑板写入操作通过 tool call 鉴权，确认来源 Agent：
...
|------|--------|---------|--------|-----------------|------------|
| **组织模型** | 单 Agent + Skills | 公司流水线 SOP | Crew 小组 | 黑板协调 | **公司架构 + 黑板涌现** |
...
| **黑板写入** | ❌ 无黑板 | ⚠️ 隐式共享 | ⚠️ 隐式共享 | ✅ 显式黑板 | ✅ 显式黑板 |
| **SOP 工作流** | ❌ 自由式 | ✅ 严格 SOP | ⚠️ 流程可选 | ❌ 自由式 | ✅ 可配置工作流 |

### Athena 的独特价值

Athena = **MetaGPT 的公司角色模型(主要架构参考)** + **Agent-Blackboard 的黑板模式** + **Hermes 的自省记忆与任务拆解(主要交互参考)** + **独创的 HR 动态招聘 + 事实分级 + 上下文完全隔离按需共享 + 会议沟通机制**
...
│  │  ┌──────────────────────────────────────────────┐   │  │
│  │  │ ChromaDB (嵌入向量索引)                       │   │  │
│  │  │ - blackboard_entries.chroma_id → 语义检索    │   │  │
│  │  │ - 写入时自动生成向量并存入ChromaDB            │   │  │
│  │  │ - 读取时支持相似度搜索                        │   │  │
│  │  └──────────────────────────────────────────────┘   │  │
│  └────────────────────────────────────────────────────┘  │
│                                                          │
...
CREATE TABLE agents (
    ...
    project_id  TEXT REFERENCES projects(id),  -- 绑定项目（执行层Agent一岗一项目；HR/AgentServer/PM可为空）
...
CREATE TABLE blackboard_entries (
    ...
    certainty   TEXT NOT NULL CHECK(certainty IN ('certain', 'conjecture')),
    author      TEXT,
    chroma_id   TEXT,           -- ChromaDB 中的向量ID（语义搜索用，向量由ChromaDB管理）
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at  DATETIME DEFAULT CURRENT_TIMESTAMP
);
```
...
> 注：此表位于独立会议 SQLite 文件内。meeting_id 保留但外键仅逻辑含义（跨库无法物理约束）。
...
| **项目经理 Agent** | 拆解细化需求、分配任务、处理不确定性上报、**验收交付** | 任务分解工具、需求分析工具 | 项目目标 + 需求文档 |
...
system_prompt: |
  ...
  发现重要事实、工具陷阱、项目约定时，写入个人记忆(memory.md)。
...
func AgentLoop(ctx context.Context, agent *Agent) error {
    for {
        ...
        case task := <-agent.TaskQueue:
            ...
            // 5. 如果 Agent 通过 tool call 写入黑板，黑板系统已处理
            // （黑板写入通过 ExecuteTool 中的 blackboard_write 工具完成，鉴权在此处执行）
...
│ 🤝 会议  │  ┌──────────────────────────────────────┐│
│  ├ 进行中 │  │ Agent 状态                             ││
│  └ 决议   │  │ 🟢 a1b2c3-dev-1   开发中  [任务3]     ││
│          │  │ 🟢 a1b2c3-test-1  测试中  [任务5]     ││
│ ⚙ 设置   │  │ 🟡 a1b2c3-review-1 审查中  [会议2]    ││
│          │  │ ⚪ a1b2c3-design-1 空闲                ││
│          │  └──────────────────────────────────────┘│
└──────────┴──────────────────────────────────────────┘
```
...
| 单日 API 费用 | < ¥100 | 10 人团队的 LLM 调用总费用 |
...
### Phase 4: 全角色 Agent + MCP工具 + 打磨 (2 周，⚠️ 乐观估计)
...
7. 验收交付（PM 是最清楚需求的 Agent，负责最终验收）
   - 验收标准：项目必须高性能运行，所有测试场景无明显可感知的 bug
   - 验收不通过 → 要求对应 Agent 整改 → 整改后重新验收
...
*最后更新: 2026-05-02 (v8: 三条核心设计哲学+黑板涌现+7处残留问题修复)*
